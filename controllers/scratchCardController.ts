...
export const getScratchCards = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if(scratchCardType == "all") {
            scratchCards = await StudentsScratchCardCollection.paginate({}, {
                ...
                populate: [
                    ...
                    {
                        path: "schoolId",
                        select: "schoolName schoolLogo schoolEmail currentTerm currentYear"
                    }
                ]
            }) as any;
        } else if(scratchCardType == "unpaired") {
            scratchCards = await StudentsScratchCardCollection.paginate({studentId: null}, {
                sort: {createdAt: -1},
                page: page ? parseInt(page) : 1,
                limit: limit ? parseInt(limit) : 10
            }) as any;
        } else if(scratchCardType == "paired") {
            scratchCards = await StudentsScratchCardCollection.paginate({studentId: {$ne: null}}, {
                sort: {createdAt: -1},
                page: page ? parseInt(page) : 1,
                limit: limit ? parseInt(limit) : 10,
                populate: [
...
export const scratchCardSummaey = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
        
        const pairedScratchCards = await StudentsScratchCardCollection.countDocuments({studentId: {$ne: null}});
        const notPairedScratchCards = await StudentsScratchCardCollection.countDocuments({studentId: null});
        const totalScratchCards = await StudentsScratchCardCollection.countDocuments();
...
export const getScratchCard = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
...
export const pairScratchCard = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
        
        const {studentId, scratchCardId} = req.body;

        const scratchCardDetails = await StudentsScratchCardCollection.findById(scratchCardId);
        ...
        const alreadyPaired = await StudentsScratchCardCollection.findOne({studentId});
        ...
        const updatedScratchCard = await StudentsScratchCardCollection.findByIdAndUpdate(scratchCardId, {
            studentId, dateIssued: new Date(), schoolId: studentDetails.schoolId
        }, {new: true});
...
export const unpairScratchCard = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const updatedScratchCard = await StudentsScratchCardCollection.findOneAndDelete({studentId, _id: scratchCardId});
...
export const searchScratchCard = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
...
export const deleteScratchCard = async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if(scratchCardDetails!!.studentId != null) {
            res.status(401).send({
                message: "Scratch card is already paired, kindly unpair before deleting"
            });
            return;
        }

        const deletedScratchCard = await StudentsScratchCardCollection.findByIdAndDelete(id);
